...
# --- Checks if a SUMO config file was passed ---
if len(sys.argv) < 2:
...
# --- Defining SUMO GUI Launch for Config file ---
Sumo_config = [
    'sumo-gui',
    '-c', sumo_config_path,
    '--step-length', '0.05',
...
PRIORITY_DURATION = 2  # How often we can overwrite the TLS in seconds (cooldown)
POST_EXIT_WAIT = 30  # Finish simulation 30 seconds after last bus exits the simulation/ finishes its route
last_priority_times = {}
last_bus_exit_time = None
bus_in_range_times = {}  # Track when each bus first gets in range of a TLS
COMM_DELAY = 9  # Communication delay between the vehicle and TLS in seconds

# --- CSV log file Setup: Priority Events ---
...
def get_tls_ahead(vehicle_id): # Finds the next TLS ahead of the bus
...
def is_near_tls(vehicle_id, tls_id): # Checks if bus is within the Distance Threshold (here 100 m)
    lane_id = traci.vehicle.getLaneID(vehicle_id)
    pos = traci.vehicle.getLanePosition(vehicle_id)
    lane_length = traci.lane.getLength(lane_id)
    return (lane_length - pos) <= DISTANCE_THRESHOLD

def should_set_priority(tls_id, sim_time): # Decides to set priority based on cooldown
    last_time = last_priority_times.get(tls_id, -999)
    return (sim_time - last_time) > PRIORITY_DURATION

def get_green_phase_for_lane(tls_id, lane_id): # This returns the green phase index for a lane at a TLS
...
try:
    # === Set buses to enter at time 0 ===
    for veh_id in traci.vehicle.getIDList():
        try:
            vehicle_type = traci.vehicle.getTypeID(veh_id)
            if vehicle_type == "bus":
                # Set the bus's departure time to 0
                traci.vehicle.setDeparture(veh_id, 0)
                print(f"Bus {veh_id} set to start at time 0.")
        except Exception as e:
            print(f"Error setting departure time for bus {veh_id}: {e}")
    
    while True:
        traci.simulationStep() # Advances simulation
        sim_time = traci.simulation.getTime() # gets simulation time
        ...
        for veh_id in active_buses: # For each active bus, checks if it is near a TLS, waits for communication delay. If eligible for a green priority and cooldown has passed, then gets the green phase index and sets that green phase.
            try:
                if veh_id not in bus_start_times:
                    bus_start_times[veh_id] = 0 if sim_time <= 0.05 else sim_time # logs start time for the bus
        ...
        finished_buses = seen_buses - active_buses - set(bus_end_times.keys()) # Buses that finished their route

        for bus_id in finished_buses: # Logs trip diration for buses that finished their route.
            end_time = sim_time
        ...
        if not active_buses: # If no bus is active in the simulation for the duration of POST_EXIT_WAIT, then end the simulation.
