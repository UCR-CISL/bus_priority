...
def estimate_eta(vehicle_id, tls_id): # Figure out the time our bus will be at the TLS
    try:
...
try:
    ...
    while True:
        ...
        for bus_id in active_buses: # For each active bus, records the start time, finds the next tls ahead and estimates the ETA to that tls for the bus. If ETA is smaller than buffer (heads-up time period) plus the commuunication delay (here it is zero), and if the cooldown has passed, then grant green priority.
            try:
