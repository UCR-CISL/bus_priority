...
COOL_OFF_PERIOD = 100  # 10s in steps
EXIT_DELAY = 300  # 30s in simulation steps
...
def run_simulation(sumoConfigFile):
    ...
    try:
        while True:
            ...
            # === Track wait times for all vehicles ===
            ...
            # === Log vehicle speeds at key lane positions ===
            for vid in traci.vehicle.getIDList():
                try:
                    vehicle_type = traci.vehicle.getTypeID(vid)
                    lane_id = traci.vehicle.getLaneID(vid)
            ...
            # === Track trip completions and last bus exit ===
            ...
            # === End simulation after last bus + delay ===
